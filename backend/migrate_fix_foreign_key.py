"""
Script para modificar la restricción de llave foránea en quiz_history
para permitir CASCADE en DELETE (establecer quiz_id a NULL cuando se elimina un quiz)
"""
import psycopg2
...
try:
    ...
    cursor.execute("""
        ...
        AND constraint_name = 'quiz_history_quiz_id_fkey'
    """)
    
    constraint_exists = cursor.fetchone()
    
    if not constraint_exists:
        print("⚠ La restricción 'quiz_history_quiz_id_fkey' no existe")
    else:
        print("Modificando restricción de llave foránea...")
        
        # Eliminar la restricción anterior
        cursor.execute("""
            ALTER TABLE quiz_history
            DROP CONSTRAINT quiz_history_quiz_id_fkey
        """)
